            ...
            <div className="card card-chart">
                {<div className="card-header border-0 align-items-center d-flex">
                    <p className="text-uppercase fw-medium text-muted text-truncate mb-0 flex-grow-1">{title}</p>
                        ...
                        {!isShared && data && <button type="button" className={"btn btn-soft-secondary btn-share material-shadow-none btn-sm d-none d-sm-block " + (allowGrouping ? "" : "no-group")} onClick={() => { onShareClick(interval); }}>
                            Share
                        </button>}
                        {allowGrouping && <div>
                            {/* eslint-disable-next-line no-implied-eval */}
